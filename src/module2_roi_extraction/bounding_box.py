"""
PHASE 3: Bounding Box Extraction
Extracts bounding boxes around filtered contours with optional padding
"""
...
def extract_bounding_boxes(contours: List[np.ndarray],
    ...
    """
    Extract bounding boxes from contours with padding
    
    Args:
        contours: List of filtered contours
        padding: Pixels to add around each bounding box (default: 10)
        image_shape: (height, width) to clip boxes within bounds
    
    Returns:
        List of (x, y, w, h) tuples with padding applied
    
    Technical Details:
        - Padding ensures defect context is captured
        - Boxes are clipped to stay within image boundaries
        - Minimum box size is enforced (padding + 1)
    """
    boxes = []
...
def compute_box_statistics(boxes: List[Tuple[int, int, int, int]]) -> dict:
    ...
    Returns:
        Dictionary with statistics:
            ...
            - avg_width: Average box width
            - avg_height: Average box height
            - avg_area: Average box area
...
def merge_overlapping_boxes(boxes: List[Tuple[int, int, int, int]],
    ...
    Useful when multiple contours belong to the same defect
    
    Args:
        boxes: List of (x, y, w, h) tuples
        iou_threshold: IoU threshold for merging (0.3 = 30% overlap)
...
def visualize_boxes(image: np.ndarray,
    ...
    Args:
        ...
        show_labels: Show box IDs (default: True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python bounding_box.py <binary_mask_path> [padding]")
        print("\nTest: Extracts and visualizes bounding boxes")
        sys.exit(1)
